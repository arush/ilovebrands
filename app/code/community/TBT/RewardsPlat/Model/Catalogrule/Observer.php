 ...
 */
class TBT_RewardsPlat_Model_Catalogrule_Observer extends TBT_Rewards_Model_Catalogrule_Observer
{
...
//	    BEGIN Rewards Only Mode Feature
		...
		$rp = Mage::getModel('rewardsplat/catalog_product')->wrap2($product);
		...
		if($rule_selector->hasRule()) {
    		if(Mage::helper('rewardsplat/config')->forceRedemptions()) {
...
//	    END Rewards Only Mode Feature
    ...
    protected function getApplyRedemptionId($request, $customer, $product) {
    	if(Mage::helper('rewardsplat/config')->showRedeemer()) {
    		$apply_rule_id = $request->getParam('redemption_rule');
    	} else {
    		$rule_selector = $this->getRedemptionSelectionAlgorithm();
