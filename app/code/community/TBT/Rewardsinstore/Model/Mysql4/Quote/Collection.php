...
class TBT_Rewardsinstore_Model_Mysql4_Quote_Collection extends Mage_Sales_Model_Mysql4_Quote_Collection
{
    ...
    {
        $this->getSelect()->columns(array($alias => "CONCAT(sales_flat_quote.customer_firstname, ' ', sales_flat_quote.customer_lastname)"));
        return $this;
    }
    ...
    {
        $this->getSelect()
            ->joinLeft('rewards_transfer_reference',
                'main_table.quote_id = rewards_transfer_reference.reference_id',
                array('reference_type', 'rewards_transfer_id'))
            ->joinLeft('rewards_transfer',
                'rewards_transfer_reference.rewards_transfer_id = rewards_transfer.rewards_transfer_id',
                array($alias => "SUM(rewards_transfer.quantity)", 'reason_id'))
            ->where('rewards_transfer_reference.reference_type = ?', TBT_Rewardsinstore_Model_Transfer_Reference_Quote::REFERENCE_TYPE_ID)
            ->where('rewards_transfer.reason_id = ?', 1)
