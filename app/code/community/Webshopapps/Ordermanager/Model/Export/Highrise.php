 ...
 * */
class Webshopapps_Ordermanager_Model_Export_Highrise extends Mage_Core_Model_Abstract
{ 
	...
	private function _query_contact($request){
		...
		if($id < 0){
			...
			curl_close($curl);
			if ($http_code = 201) {
				$request['status'] =   Webshopapps_Ordermanager_Model_Export_Status::STATUS_CREATED;
				$id = $this->_person_in_highrise($request);
				...
				$request['status'] =   Webshopapps_Ordermanager_Model_Export_Status::STATUS_FAILED;
			}
		}elseif($this->highrise_notes && !$this->_order_in_highrise($request,$id)){
		    $updateStatus = $this->_order_to_notes($request,$id);
		    if ($updateStatus == 201) {
		    	$request['status'] = Webshopapps_Ordermanager_Model_Export_Status::STATUS_UPDATED;
		    }
		    else {
		    	$request['status'] =   Webshopapps_Ordermanager_Model_Export_Status::STATUS_FAILED;
		    }
			...
			$request['status'] =   Webshopapps_Ordermanager_Model_Export_Status::STATUS_DUPLICATE;
		}
		return $request;
	}
