<?php
class Evogue_Customer_Model_Observer {
    ...
    public function salesQuoteAddressCollectionAfterLoad(Varien_Event_Observer $observer)
    {
        ...
        if ($collection instanceof Varien_Data_Collection_Db) {
            Mage::getModel('evogue_customer/sales_quote_address')
    ...
    public function salesOrderAfterLoad(Varien_Event_Observer $observer)
    {
        ...
        if ($order instanceof Mage_Core_Model_Abstract) {
            Mage::getModel('evogue_customer/sales_order')
                ->load($order->getId())
    ...
    public function salesOrderAddressCollectionAfterLoad(Varien_Event_Observer $observer)
    {
        ...
        if ($collection instanceof Varien_Data_Collection_Db) {
            Mage::getModel('evogue_customer/sales_order_address')
                ->attachDataToCollection($collection);
        }
     ...
     * @return Evogue_Customer_Model_Observer
     */
    public function evogueCustomerAttributeSave(Varien_Event_Observer $observer)
    {
     ...
     * @return Evogue_Customer_Model_Observer
     */
    public function evogueCustomerAttributeDelete(Varien_Event_Observer $observer)
    {
        $attribute = $observer->getEvent()->getAttribute();
        if ($attribute instanceof Mage_Customer_Model_Attribute
            && !$attribute->isObjectNew()
     ...
     * @return Evogue_Customer_Model_Observer
     */
    public function evogueCustomerAddressAttributeSave(Varien_Event_Observer $observer)
    {
        $attribute = $observer->getEvent()->getAttribute();
        if ($attribute instanceof Mage_Customer_Model_Attribute
            && $attribute->isObjectNew()
     ...
     * @return Evogue_Customer_Model_Observer
     */
    public function evogueCustomerAddressAttributeDelete(Varien_Event_Observer $observer)
    {
        $attribute = $observer->getEvent()->getAttribute();
        if ($attribute instanceof Mage_Customer_Model_Attribute
            && !$attribute->isObjectNew()
    ...
    protected function _copyFieldset(Varien_Event_Observer $observer, $algoritm, $convertType)
    {
        ...
        if ($source instanceof Mage_Core_Model_Abstract && $target instanceof Mage_Core_Model_Abstract) {
            if ($convertType == self::CONVERT_TYPE_CUSTOMER) {
                $attributes = Mage::helper('evogue_customer')->getCustomerUserDefinedAttributeCodes();
                $prefix     = 'customer_';
            } else if ($convertType == self::CONVERT_TYPE_CUSTOMER_ADDRESS) {
                $attributes = Mage::helper('evogue_customer')->getCustomerAddressUserDefinedAttributeCodes();
                $prefix     = '';
            } else {
                return $this;
            }
