...
function top_exclude() {
    if (current_user_can('manage_options')) 
        {
    $message = null;
    $message_updated = __("Tweetily Options Updated.", 'Tweetily');
    $response = null;
    $records_per_page = 20;
    ...
    if (isset($_POST["excludeall"])) {
        if (substr($_POST["delids"], 0, -1) == "") {
			...
			<div id="message" style="margin-top:30px" class="updated fade">
				<p>' . __('No post selected please select a post to be excluded.', 'Tweetily') . '</p>
			</div>');
        } else {
            ...
            print('
			<div id="message" style="margin-top:30px" class="updated fade">
				<p>' . __('Posts excluded successfully.', 'Tweetily') . '</p>
			</div>');
        }
    }
    ...
    print('
			<div id="help" style="overflow: hidden;padding-left:15px; font-size: 14px; margin-top: 25px; background-color: rgb(239, 239, 239);">
				<p>' . __('This page shows all the post from your <a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=Tweetily">selected categories</a>.<br /> All the post excluded will not be tweeted even if the category is selected.', 'Tweetily') . '</p>
			</div>');
    ...
    print('
			<div id="help" style="overflow: hidden;padding-left:15px; font-size: 14px; margin-top: 25px;margin-bottom:25px; background-color: rgb(239, 239, 239);">
				<p>' . __('You have selected following POST IDs to be excluded from tweeting: <span id="excludeList" style="font-weight:bold;font-style:italic;"></span>.<br /> <strong>Note:</strong> If you have made any change and dint hit "Exclude Selected" button changes will not be saved. Please hit "Exclude Selected" button after making any changes.', 'Tweetily') . '</p>
			</div>');
...
if($excludeList.length >0)
{
 ...
 print('<form id="top_TweetOldPost" name="top_TweetOldPost" action="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=ExcludePosts" method="post"><input type="hidden" name="delids" id="delids" value="' . $exposts . '" /><input type="submit" id="pageit" name="pageit" style="display:none" value="" /> ');
        ...
        print('<input type="submit" class="button-secondary" name="excludeall" value="' . __($update_text, 'Tweetily') . '" />');
	...
	wp_dropdown_categories($dropdown_options);
        print('<input type="submit" class="button-secondary" name="setFilter" value="' . __('Filter', 'Tweetily') . '" />');
    ...
    if (count($posts) > 0) {
        ...
        print('<div class="tablenav"><div class="alignleft actions"><input type="submit" class="button-secondary" name="excludeall" value="' . __($update_text, 'Tweetily') . '" /></div>');
...
else
{
    ...
    print('
			<div id="message" style="margin-top:30px" class="updated fade">
				<p>' . __('No Posts found. Review your search or filter criteria/term.', 'Tweetily') . '</p>
			</div>');
}
print('</form>');
} else {
        print('
			<div id="message" class="updated fade">
				<p>' . __('You do not have enough permission to set the option. Please contact your admin.', 'Tweetily') . '</p>
			</div>');
    }
}
?>