...
/**
 * Observes events on the admin/session model
 *
 * @category   TBT
 * @package    TBT_Rewardsinstore
...
class TBT_Rewardsinstore_Model_Admin_Session_Observer
{
    ...
    public function userLogin($observer)
    {
        ...
        if ($request->getRouteName() == 'rewardsinstoreadmin') {
            if ($request->getControllerName() == 'webapp_ajax') {
                if ($request->getActionName() == 'main') {
