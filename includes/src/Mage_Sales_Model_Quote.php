...
class Mage_Sales_Model_Quote extends Mage_Core_Model_Abstract
{
    ...
    public function addItem(Mage_Sales_Model_Quote_Item $item)
    {
        ...
        if ($item->isNominal() && $this->hasItems() || $this->hasNominalItems()) {
            Mage::throwException( 
            /***** Arush edited for BRANDiD ******/
                Mage::helper('sales')->__('We\'re sorry, Paypal are being lame… you need to empty your basket before adding a plan. Paypal can only handle processing a plan on its own. We\'re working hard to build a brand spanking new billing system, but for now please just checkout your plan individually from any other plan or booster.')
            );
        }
