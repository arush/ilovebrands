 ...
 */
class TBT_RewardsPlat_Model_Catalog_Product extends TBT_Rewards_Model_Catalog_Product
{
	 ...
	 * @return TBT_RewardsPlat_Model_Catalog_Product
	 */
	public function wrap2(Mage_Catalog_Model_Product $product) {
       	$rewards_product = Mage::getModel('rewardsplat/catalog_product')
...
//    		$rule_id = $rule->getId();
    		...
    		$rule = TBT_RewardsPlat_Model_Catalogrule_Rule::wrap($rule);
    	} elseif((int)$rule > 0) {
    		$rule_id = $rule;
	...
	public function getSimplePointsCost($customer, $positive=true) {
		...
		$rule_selector = Mage::helper('rewardsplat/config')->getRedemptionSelectionAlgorithm();
