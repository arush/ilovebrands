...
/**
 * Provides a bunch of helper methods to make managing Customer data a lot easier
 *
 * @category   TBT
 * @package    TBT_Rewardsinstore
...
class TBT_Rewardsinstore_Helper_Customer extends Mage_Core_Helper_Abstract
{
    ...
    public function retrieveCustomerData($customerId)
    {
        ...
        $data = file_exists($file) ? Zend_Json::decode(base64_decode(file_get_contents($file))) : array();
        $text = isset($data[$customerId]) ? $data[$customerId] : $this->__("(Inaccessible.  Please contact store owner.)");
