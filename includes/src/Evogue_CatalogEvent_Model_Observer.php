<?php
class Evogue_CatalogEvent_Model_Observer {
        ...
        $collection->addFieldToFilter('status',
            array(
                'in' => array(
                    ...
                    Evogue_CatalogEvent_Model_Event::STATUS_UPCOMING
                )
            ));
    ...
    public function applyIsSalableToProduct(Varien_Event_Observer $observer) {
        ...
        if ($event && in_array($event->getStatus(), array(
                    Evogue_CatalogEvent_Model_Event::STATUS_CLOSED,
                    Evogue_CatalogEvent_Model_Event::STATUS_UPCOMING
        ))) {
    ...
    public function applyEventOnQuoteItemSetQty(Varien_Event_Observer $observer) {
        ...
        if ($item->getEventId()) {
            if ($event = $item->getEvent()) {
                if ($event->getStatus() !== Evogue_CatalogEvent_Model_Event::STATUS_OPEN) {
                    $item->setHasError(true)
                        ->setMessage(
                            Mage::helper('evogue_catalogevent')->__('Sale was closed for this product.')
                        );
                    $item->getQuote()->setHasError(true)
                        ->addMessage(
                            Mage::helper('evogue_catalogevent')->__('Some products are not salable anymore.')
                        );
                }
            } else {
    ...
    protected function _getProductEvent($product) {
        ...
        foreach ($categoryIds as $categoryId) {
            ...
            if ($categoryEvent === false) {
                ...
                return null;
            } elseif ($categoryEvent->getStatus() == Evogue_CatalogEvent_Model_Event::STATUS_OPEN) {
               $event = $categoryEvent;
            } else {
    ...
    protected function _getEventInStore($categoryId) {
        ...
        if ($this->_eventsToCategories === null) {
            $this->_eventsToCategories = Mage::getModel('evogue_catalogevent/event')->getCategoryIdsWithEvent(
