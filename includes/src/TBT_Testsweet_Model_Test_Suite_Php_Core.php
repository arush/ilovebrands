...
class TBT_Testsweet_Model_Test_Suite_Php_Core extends TBT_Testsweet_Model_Test_Suite_Abstract {
    ...
    protected function generateSummary() {
        ...
        if (ini_get('max_execution_time') == 0) {
            $this->addWarning($this->__("Max execution time is set to forever. PHP max_execution_time reports: %s.", ini_get('max_execution_time')), $this->__("If a faulty subroutine is stuck in a loop then the resources might never be released untill the service is reset.  You should set the max_execution_time to 3600 to mitigate this issue."));
        } elseif (ini_get('max_execution_time') >= 3600) {
            ...
            $this->addFail($this->__("Max execution time is less than 600 seconds (10 min) PHP reports: %s seconds.", ini_get('max_execution_time')), $this->__("Some tasks can require extra time to complete.  This is offen seen as a timeout message in your web browser.  You are advised to set the PHP max_execution_time to 3600."));
        }
    }
